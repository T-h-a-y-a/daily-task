...
import { LogOut, Plus, LayoutDashboard, Calendar, Users, Globe, Download, MoreVertical, FileText } from 'lucide-react';
...
export default function Dashboard({ user, onLogout }) {
  const [activeTab, setActiveTab] = useState('seo_tasks'); // 'seo_tasks', 'website_tasks', 'report_tasks', 'users'
  ...
  const unreadReportCount = getUnreadCount('report');
  ...
  const handleAddTask = async (taskData) => {
    const newTask = {
      ...taskData,
      type: activeTab === 'seo_tasks' ? 'seo' : activeTab === 'website_tasks' ? 'website' : 'report',
  ...
  const handleExport = () => {
    const currentTasks = tasks.filter(t => t.type === (activeTab === 'seo_tasks' ? 'seo' : activeTab === 'website_tasks' ? 'website' : 'report'));
    ...
    XLSX.writeFile(wb, `${activeTab === 'seo_tasks' ? 'SEO' : activeTab === 'website_tasks' ? 'Website' : 'Report'}_Tasks_Report.xlsx`);
  };
    ...
    <div className="resp-layout">
      ...
      <aside className="glass-panel resp-sidebar">
        ...
        <nav style={styles.nav} className={`resp-nav ${isMobileMenuOpen ? 'open' : ''}`}>
          ...
          <div 
            style={activeTab === 'report_tasks' ? styles.navItemActive : styles.navItem}
            onClick={() => { setActiveTab('report_tasks'); setIsMobileMenuOpen(false); }}
          >
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
              <FileText size={20} />
              <span>Report Task</span>
            </div>
            {unreadReportCount > 0 && <span style={styles.badge}>{unreadReportCount}</span>}
          </div>
      ...
      <main className="resp-main">
        {reportFilesTask ? (
          ...
          />
        ) : activeTab === 'seo_tasks' || activeTab === 'website_tasks' || activeTab === 'report_tasks' ? (
          <>
            <header className="resp-header">
              <div>
                <h1 style={styles.pageTitle}>{activeTab === 'seo_tasks' ? 'SEO Tasks' : activeTab === 'website_tasks' ? 'Website Tasks' : 'Report Tasks'}</h1>
            ...
            <div className="animate-fade-in" style={styles.content}>
              <TaskBoard
                tasks={tasks.filter(t => t.type === (activeTab === 'seo_tasks' ? 'seo' : activeTab === 'website_tasks' ? 'website' : 'report'))}
                currentUser={user}
                isAdmin={isAdmin}
      ...
      {/* Modals */}
      {isModalOpen && (activeTab === 'seo_tasks' || activeTab === 'website_tasks' || activeTab === 'report_tasks') && (
        <TaskModal
          task={editingTask}
          isAdmin={isAdmin}
          onClose={() => {
      ...
      {isCommentModalOpen && (activeTab === 'seo_tasks' || activeTab === 'website_tasks' || activeTab === 'report_tasks') && (
        <CommentModal
          task={editingTask}
          currentUser={user}
          onClose={() => {
