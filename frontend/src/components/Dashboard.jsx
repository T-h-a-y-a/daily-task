...
import { LogOut, Plus, LayoutDashboard, Calendar, Users, Globe, Download, MoreVertical, FileText, FolderKanban, Folder, Layers, Edit2, Trash2, X, ChevronDown, ChevronRight, ShieldCheck, ArrowLeft, Menu, Loader, CheckCircle2 } from 'lucide-react';
...
export default function Dashboard({ user, onLogout }) {
  ...
  // Custom categories list (Main Headings) with localStorage persistence
  const [customCategories, setCustomCategories] = useState(() => {
    try {
      const saved = localStorage.getItem('seo_custom_categories');
      return saved ? JSON.parse(saved) : ['SEO', 'Website Development', 'Reports'];
    } catch {
      return ['SEO', 'Website Development', 'Reports'];
    }
  });

  // Custom sub-projects list by category with localStorage persistence
  const [customProjectsByCategory, setCustomProjectsByCategory] = useState(() => {
    try {
      const saved = localStorage.getItem('seo_custom_projects');
  ...
  // Save changes to localStorage whenever categories or sub-projects are created, renamed, or deleted
  useEffect(() => {
    try {
      localStorage.setItem('seo_custom_categories', JSON.stringify(customCategories));
    } catch (e) {
  ...
  useEffect(() => {
    try {
      localStorage.setItem('seo_custom_projects', JSON.stringify(customProjectsByCategory));
    } catch (e) {
  ...
  const handleCreateCategorySubmit = (e) => {
    ...
    showToast(`Main Heading "${name}" created successfully!`);
  };
  ...
  const handleRenameCategorySubmit = async (e) => {
    ...
    try {
      ...
      showToast('Main Heading updated successfully!');
    } catch (err) {
  ...
  const handleDeleteCategoryConfirm = async () => {
    ...
    try {
      ...
      showToast('Main Heading deleted successfully!');
    } catch (err) {
  ...
  const handleRenameProjectSubmit = async (e) => {
    ...
    try {
      ...
      setTasks(prev => prev.map(t => (t.project || '') === projectToEdit ? { ...t, project: newName } : t));
      setCustomProjectsByCategory(prev => {
        const updated = {};
        Object.keys(prev).forEach(cat => {
          updated[cat] = (prev[cat] || []).map(p => p === projectToEdit ? newName : p);
        });
        return updated;
      });
      ...
      showToast('Sub-Project updated successfully!');
    } catch (err) {
  ...
  const handleDeleteProjectConfirm = async () => {
    ...
    try {
      ...
      setCustomProjectsByCategory(prev => {
        const updated = {};
        Object.keys(prev).forEach(cat => {
          updated[cat] = (prev[cat] || []).filter(p => p !== projectToDelete);
        });
        return updated;
      });
      ...
      showToast('Sub-Project deleted successfully!');
    } catch (err) {
  ...
  const handleAddTask = async (taskData) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        showToast(`Task "${savedTask.title || 'Task'}" created successfully!`);
      }
    } catch (err) {
  ...
  const handleDeleteTask = async (id) => {
    ...
    try {
      ...
      if (res.ok) {
        setTasks(prevTasks => prevTasks.filter((t) => t.id !== id && t._id !== id));
        showToast('Task deleted successfully!');
      } else {
        alert('Failed to delete task from server database.');
      }
    } catch (err) {
  ...
  const handleExport = () => {
    let exportTasks = [];
    if (selectedView.type === 'ALL' || selectedView.type === 'USERS') {
      exportTasks = tasks;
    } else if (selectedView.type === 'CATEGORY') {
      exportTasks = tasks.filter(t => (t.category || 'SEO') === selectedView.name);
    } else if (selectedView.type === 'PROJECT') {
      exportTasks = tasks.filter(t => (t.project || 'General') === selectedView.name);
    } else {
      exportTasks = tasks;
    }

    if (exportTasks.length === 0) {
      alert("No tasks available to export in this section.");
      return;
    }

    const wb = XLSX.utils.book_new();

    const formatTaskCommentsText = (task) => {
      if (Array.isArray(task.comments) && task.comments.length > 0) {
        const sorted = [...task.comments].sort((a, b) => new Date(a.createdAt) - new Date(b.createdAt));
        return sorted.map((c, idx) => {
          const dateStr = c.createdAt 
            ? `${new Date(c.createdAt).toLocaleDateString()} ${new Date(c.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}` 
            : '';
          const author = c.authorName || 'Unknown';
          const file = c.fileName ? ` [Attached File: ${c.fileName}]` : '';
          return `${idx + 1}. [${author}] (${dateStr}): ${c.text || ''}${file}`;
        }).join('\n');
      } else if (task.comments && typeof task.comments === 'string' && task.comments.trim() !== '') {
        return task.comments.trim();
      }
      return 'No comments';
    };

    // Extract all distinct Main Headings (Categories) in exportTasks
    const exportCategories = Array.from(new Set(exportTasks.map(t => t.category || 'SEO')));

    // Build a Separate Sheet for Each Main Heading
    exportCategories.forEach(catName => {
      const catTasks = exportTasks.filter(t => (t.category || 'SEO') === catName);
      if (catTasks.length === 0) return;

      const sheetData = []; // Array of rows for AOA format

      // Group tasks by Sub-Heading (Sub-Project)
      const subProjectsInCat = Array.from(new Set(catTasks.map(t => t.project || 'General')));

      subProjectsInCat.forEach((projName) => {
        const projTasks = catTasks.filter(t => (t.project || 'General') === projName);
        if (projTasks.length === 0) return;

        // 1. Sub-Heading Header Banner Row
        sheetData.push([`SUB-PROJECT: ${projName.toUpperCase()}`]);

        // 2. Table Columns Header Row
        sheetData.push([
          'Task Title',
          'Description',
          'Status',
        ...
        // 3. Task Rows
        projTasks.forEach(t => {
          const dropUrl = t.dropboxUrl || t.dropboxLink || t.fileUrl || t.link || t.url || '';
          const fileNames = t.files && t.files.length > 0 ? t.files.map(f => f.name).join(', ') : 'None';
          const commentsCount = Array.isArray(t.comments) ? t.comments.length : (t.comments && t.comments.trim() !== '' ? 1 : 0);
          const commentsText = formatTaskCommentsText(t);

          sheetData.push([
            t.title || '',
            t.description || '',
            t.status || 'pending',
            t.frequency || t.type || 'N/A',
            dropUrl || 'None',
        ...
        // 4. One Blank Line after each Sub-Heading and its data
        sheetData.push([]);
      });
      ...
      if (sheetData.length > 0) {
        ...
        ws['!cols'] = [
          ...
          { wch: 45 }, // Description
          { wch: 16 }, // Status
          { wch: 18 }, // Frequency/Type
          { wch: 35 }, // Link
          { wch: 15 }, // Date Created
          { wch: 15 }, // Created By
          { wch: 25 }, // Attached Files
    ...
    // Optional Final Sheet: Comments Breakdown
    const commentsData = [];
      ...
      if (Array.isArray(t.comments) && t.comments.length > 0) {
        ...
        sortedComments.forEach((c, idx) => {
          commentsData.push({
            'Comment #': idx + 1,
            'Task Title': t.title || '',
            'Main Heading / Category': t.category || 'SEO',
            'Sub-Project': t.project || 'General',
            'Comment Text': c.text || '',
    ...
    if (commentsData.length > 0) {
      ...
      XLSX.utils.book_append_sheet(wb, wsComments, "Comments Breakdown");
    }

    const viewName = selectedView.type !== 'ALL' && selectedView.name 
      ? selectedView.name.replace(/[^a-zA-Z0-9_-]/g, '_')
      : 'All_Projects';
    const exportFileName = `Project_Status_Report_${viewName}.xlsx`;
    ...
    <div className="resp-layout">
      ...
      <aside className="glass-panel resp-sidebar">
        <div style={styles.logoArea} className="resp-logo-area">
          <h2 style={{ ...styles.logoText, fontSize: '1.15rem', lineHeight: '1.2' }}>Project Progress Tracker</h2>
          <button 
            ...
            <>
              ...
              <div style={styles.projectsList}>
                ...
                {availableCategories.map(cat => {
                  const isReportsCat = cat.toLowerCase() === 'reports';
                  const catTasks = tasks.filter(t => t.category === cat && t.type !== 'report');
                  const catProjects = isReportsCat ? [] : Array.from(new Set([
                  ...
                  const catBadgeCount = isReportsCat 
                    ? tasks.filter(t => t.type === 'report').length 
                    : catProjects.length;
                    ...
                    <div key={cat} style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                      ...
                      >
                        <div style={{ display: 'flex', alignItems: 'center', gap: '6px', overflow: 'hidden', flex: 1 }}>
                          ...
                          <FolderKanban size={15} style={{ color: isCatSelected ? 'var(--accent-primary)' : '#818cf8', flexShrink: 0, marginLeft: isReportsCat ? '4px' : '0' }} />
                          <span style={styles.categoryTitleText}>{cat}</span>
                        </div>

                        <span style={styles.categoryBadge}>{catBadgeCount}</span>
                      </div>

                      {/* Sub-Projects List (Hidden for Reports category) */}
                      {!isReportsCat && isExpanded && (
